...
try:
    ...
    # We will find the entire 'const promptStages = [...];' block using regex and replace it
    pattern = r'const promptStages = \[\s*\{.*?\}\s*\n  \];'
  ...
  const promptStages = dbStages.length ? dbStages : [
    ...
    content = re.sub(pattern, replace_stages, content, flags=re.DOTALL)
    
    io.open('frontend/src/App.jsx', 'w', encoding='utf-8').write(content)
    print("Patched promptStages array correctly")
